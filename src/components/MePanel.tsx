...
export default function MePanel(props: Props) {
  ...
  const hasSources = Boolean(sources.archive || sources.resume || sources.mailbox);
  const name = sources.archive?.verifiedAccountHolder?.fullName;
  return <>
    <header className="page-heading with-actions"><div><h1>{tab === 'Profile' ? 'Your profile' : tab === 'Goal' ? 'Your goal' : tab === "Things you've learned" ? "Things you've learned" : 'Settings'}</h1></div></header>
    <Tabs label="Me section" items={meTabs} value={tab} onChange={onTab}/>
    <div className="me-content">
      {(tab==='Settings'||tab==="Things you've learned")&&props.devicePanel}
      {tab === 'Profile' && (!hasSources ? <section className="profile-empty gradient-card"><h2>No sources on this device yet.</h2><p>Import your LinkedIn archive, resume, or mailbox.</p><button className="button primary" onClick={() => onTab("Things you've learned")}>Import sources<ArrowRight size={16}/></button></section> : <>
        {sources.archive ? <section className="profile-overview gradient-card"><div className="section-heading"><p className="eyebrow">{name ? `${name} · Your sources` : 'What your sources hold'}</p>{sources.archive && <span className="muted small">Imported {formatDate(sources.archive.layer1.importedAt)}</span>}</div><div className="source-counts"><div className="source-orb primary-orb"><strong>{props.connectionCount.toLocaleString()}</strong><span>imported connections</span></div><div className="source-orb coral-orb"><strong>{sources.archive.writingSamples.length}</strong><span>own writing samples</span></div><div className="source-orb pale-orb"><strong>{sources.archive?.counts.positions || 0}</strong><span>recorded positions</span></div><div className="source-orb green-orb"><strong>{sources.archive.counts.skills}</strong><span>recorded skills</span></div></div><p className="muted small">From your LinkedIn archive.</p></section> : <section className="panel content-panel"><h2>Imported on this device</h2>{sources.resume && <p>Resume · {sources.resume.pages} pages</p>}{sources.mailbox && <p>Mailbox · {sources.mailbox.summary.counts.messages.toLocaleString()} messages · {sources.mailbox.samples.length} own writing samples</p>}</section>}
        {Boolean(sources.archive?.layer1.positions.length) && <section className="panel content-panel"><p className="eyebrow">Career history</p><div className="career-facts">{sources.archive!.layer1.positions.map((position, index) => <article key={index}><span className="career-dot"/><div><h3>{position.Title || position.Position || 'Position in your archive'}</h3><p>{position['Company Name'] || position.Company || ''}</p>{(position['Started On'] || position['Finished On']) && <small>{[position['Started On'], position['Finished On']].filter(Boolean).join(' — ')}</small>}</div></article>)}</div></section>}
        {sources.resume && <section className="panel content-panel"><div className="section-heading"><h2>Your resume</h2><span className="pill neutral">{sources.resume.pages} pages</span></div><details className="source-details"><summary>Read the imported text</summary><p className="preserve-text">{sources.resume.text}</p></details></section>}
        <button className="text-button" onClick={() => onTab("Things you've learned")}>Manage sources<ArrowRight size={14}/></button>
      </>)}
      {tab === 'Goal' && <section className="panel content-panel goal-editor"><p>What do you want to achieve, and who could help?</p><form onSubmit={event => {event.preventDefault(); void props.onSaveStrategy();}}><label htmlFor="goal">Your goal</label><textarea id="goal" value={strategy} onChange={event => props.onStrategy(event.target.value)} rows={7} maxLength={16000} placeholder="I want to meet product leaders building useful tools for…"/><div className="row-actions"><button className="button primary" disabled={busy}>Save goal</button><button type="button" className="button secondary" onClick={props.onExplore}>Explore your network</button></div><p className="muted small">Your words are saved on this device as you type.</p></form></section>}
      {tab === "Things you've learned" && <><div className="source-grid">
        <section className="panel source-card"><span className="source-icon"><Users size={22}/></span><h2>LinkedIn archive</h2><p>{sources.archive ? `${sources.archive.counts.connections.toLocaleString()} connections · ${sources.archive.counts.positions} positions · ${sources.archive.counts.threads.toLocaleString()} threads` : 'Connections and career history.'}</p><p className="muted small">Your archive includes other people’s messages. Received bodies are discarded before saving.</p><FileButton label="Choose archive ZIP" accept=".zip" disabled={busy} onFile={props.onArchive}/><button className="text-button" disabled={busy || !uid} onClick={() => void props.onRebuild()}>Rebuild from my archive</button></section>
        <section className="panel source-card"><span className="source-icon coral"><FileText size={22}/></span><h2>Resume</h2><p>{sources.resume ? `${sources.resume.pages} pages read. Original facts preserved.` : 'A PDF with selectable text.'}</p><FileButton label="Choose resume PDF" accept=".pdf,application/pdf" disabled={busy} onFile={props.onResume}/></section>
        <section className="panel source-card"><span className="source-icon coral"><Mail size={22}/></span><h2>Mailbox</h2><p>Your Google Takeout .mbox file.</p><p className="muted small">The mailbox stays on this device. Only tallies and up to forty samples of your own writing can be saved.</p><label htmlFor="mailbox-addresses">Your email address or aliases</label><input id="mailbox-addresses" value={props.ownEmail} onChange={event => props.onOwnEmail(event.target.value)} placeholder="me@example.com" autoComplete="email"/><FileButton label="Choose mailbox" accept=".mbox" disabled={busy} onFile={props.onMailbox}/>{sources.mailbox && <p className="muted small">{sources.mailbox.summary.counts.messages.toLocaleString()} messages · {sources.mailbox.summary.globalMetrics.bidirectionalContacts.toLocaleString()} two-way contacts · {sources.mailbox.samples.length} own writing samples</p>}</section>
