...
export default function App(){
 ...
 const newPerson=async(p:Connection,reason:string)=>{if(!isCurrent())throw Error('Your account changed. Please try again.');await savePerson(uid,{person:p.person,url:p.profile_url,reason,company:p.company,position:p.position||p.role,photoUrl:p.photoUrl,source:p.context?.source==='web_search'?'web_search':'discovery',searchHeadline:typeof p.context?.searchHeadline==='string'?p.context.searchHeadline:undefined,searchSnippet:typeof p.context?.searchSnippet==='string'?p.context.searchSnippet:undefined});await refreshAfterWrite(`${p.person} saved.`);};
   ...
   <div className={`content content-${page.toLowerCase()}`}>
    ...
    {page === 'Today' && <>
     ...
     <section className="start-card">
      {featuredPerson ? <Avatar name={featuredPerson.person} photoUrl={personPhotoUrl(featuredPerson)} tone={1}/> : <span className="start-symbol"><Users size={23}/></span>}
