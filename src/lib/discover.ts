...
export type Connection={id?:string;person:string;profile_url?:string|null;company?:string;position?:string;role?:string;connectedOn?:string;context?:Record<string,unknown>};
export type NetworkMatch={person:Connection;reason:string;matchedTerms:string[];sharedEmployer:string|null;connectionAgeDays:number|null};
...
export function ownNetwork(query:string,all:Connection[],strategy='',employers:string[]=[],now=Date.now()):NetworkMatch[]{
 const terms=queryTerms(query);if(!terms.length)return [];
 const goals=new Set(queryTerms(strategy));const shared=new Map(employers.map(s=>[s.normalize('NFKC').trim().toLowerCase(),s]));
 return all.flatMap(person=>{const position=person.position||person.role||'';const tokens=new Set(words(`${position} ${person.company||''}`));const matched=terms.filter(t=>tokens.has(t));if(!matched.length)return [];
  const employer=shared.get((person.company||'').normalize('NFKC').trim().toLowerCase())||null;
  ...
  return [{person,reason:parts.join(' '),matchedTerms:matched,sharedEmployer:employer,connectionAgeDays:age,order:matched.length*100+goalTerms.length*10+(employer?5:0)+(age!==null&&age>365?1:0)}];
...
const relevant=/\b(network|networking|relationship|relationships|contact|contacts|introduce|introduction|connect|connection|connections|meet|meeting|follow.?up|reach.?out|talk to|know|hiring|hire|fundraising|investor|advisory|advisor|partnership|partner|career|colleague|mentor|strategy)\b/i;
export async function askMighty(question:string,all:Connection[],strategy:string,employers:string[],call:GatewayCall){
 if(!relevant.test(question))throw Error('Ask a question about your professional relationships, network, or networking strategy.');
