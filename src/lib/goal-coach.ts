...
function contextInput(value: GoalCoachContext): GoalCoachContext {
  if (!kinds.includes(value.kind)) throw invalid();
  const criteria = array(value.criteria, 30).map(normalizeGoalCriterion);
  if (new Set(criteria.map(c => c.id)).size !== criteria.length) throw invalid();
  return {kind: value.kind, title: readable(value.title, 200, true), outcome: readable(value.outcome, 16000, true),
    criteria, openQuestions: array(value.openQuestions, 30).map(q => readable(q, 1000))};
}
...
export function parseGoalCoachReply(raw: string, context: GoalCoachContext, messages: readonly GoalCoachMessage[]): GoalCoachReply {
  try {
    ...
    if (decoded.proposal === null) return {message, proposal: null};
    ...
    const proposal = contextInput(value as GoalCoachContext);
    ...
    const knownNumbers = new Set(numericTokens([context.title, context.outcome, ...context.criteria.flatMap(c => [c.label, ...c.terms]), ...messages.filter(m => m.role === 'user').map(m => m.text)].join('\n')));
