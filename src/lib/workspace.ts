import { db, accountId, gateway, saveSettings } from './platform';
...
export interface LocalSources { archive?:ArchiveResult;resume?:ResumeExtraction;mailbox?:Record<string,unknown>;strategy?:string;knowledge?:KnowledgeState }
...
export function keepLocal(key:string,patch:LocalSources){const job=localChain.catch(()=>{}).then(async()=>{const current=await localSources(key);const store=await openLocal();try{await new Promise<void>((resolve,reject)=>{const tx=store.transaction('sources','readwrite');tx.objectStore('sources').put({...current,...patch},key);tx.oncomplete=()=>resolve();tx.onerror=()=>reject(tx.error);tx.onabort=()=>reject(tx.error);});}finally{store.close();}});localChain=job;return job;}
...
export async function savePerson(input:{person:string;url?:string|null;reason:string;company?:string;position?:string;source?:string}){
 const uid=await accountId();const profile_url=canonicalProfile(input.url||'');const person=cleanText(input.person).trim();if(!person||person.length>200)throw Error('Enter a name under 200 characters.');
...
export async function capture(personId:string,kind:string,body='',relatedId?:string){const uid=await accountId();const {error}=await db!.from('outreach_events').insert({user_id:uid,relationship_id:personId,kind,body:cleanText(body),related_event_id:relatedId||null});if(error)throw Error(error.message);}
export async function changeStage(personId:string,stage:string){const uid=await accountId();const {error}=await db!.from('outreach_log').update({stage}).eq('user_id',uid).eq('id',personId);if(error)throw Error(error.message);}
...
export async function saveResume(resume:ResumeExtraction){const uid=await accountId();await saveSource(uid,'resume',resume.fingerprint,{text:cleanText(resume.text),pages:resume.pages});}
export async function saveMailbox(summary:Record<string,unknown>){const uid=await accountId();await saveSource(uid,'mailbox',await contentFingerprint(summary),summary);}
export async function saveArchive(archive:ArchiveResult,onProgress:(saved:number)=>void){
 const uid=await accountId();const hash=await contentFingerprint({uid,archive:archive.fingerprint});const id=`${hash.slice(0,8)}-${hash.slice(8,12)}-4${hash.slice(13,16)}-a${hash.slice(17,20)}-${hash.slice(20,32)}`;
...
export async function savedArchiveBlob(){const uid=await accountId();const {data,error}=await db!.from('imports').select('storage_path').eq('user_id',uid).eq('kind','linkedin_archive').eq('status','completed').order('created_at',{ascending:false}).limit(1).maybeSingle();if(error)throw Error(error.message);if(!data?.storage_path)throw Error('No saved archive is available yet.');const result=await db!.storage.from('archives').download(data.storage_path);if(result.error)throw Error(result.error.message);return result.data;}
export function synthesizer(key:string){return createKnowledgeSynthesizer({gateway,read:async()=>((await localSources(key)).knowledge||null),persist:async knowledge=>{await saveSettings({knowledge});await keepLocal(key,{knowledge});return {error:null};}});}
