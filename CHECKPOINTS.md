...
| Release-source checks | 403 release core tests and 120 UI checks pass. Extension 0.2.5 passes 107 checks. App and extension production packages build. | The full working directory has 427 core checks because it also contains 24 tests for the uncommitted Tavily adapter. Those 24 are excluded from the release-source count. |
...
| Extension | Installed 0.2.4 connected to the actual signed-in Chrome account after the supplied extension ID was entered. Version 0.2.5 adds explicit pairing from the popup, safe stage-specific connection errors and current SDUI profile-heading support scoped to the correct profile region. A failed page read no longer discards a verified account connection. | The new package requires Chrome reload. Native LinkedIn profile read → Save → app acceptance remains unverified. Chrome controls cannot operate internal extension pages. No new permissions, background message monitoring or automatic Connect/Message actions. |
